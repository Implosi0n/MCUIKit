...
- (void)tapped {
	...
	keyboardViewController.text = [[self.label.text mutableCopy] autorelease];
	[[[UIApplication sharedApplication].delegate.window rootViewController] presentModalViewController:keyboardViewController animated:YES];
...
- (instancetype)initWithFrame:(CGRect)frame callback:(void (^)(MCUITextField*, NSString*))callback {
    if (self = [super initWithFrame:frame]) {
		...
		self.label = [[[MCUILabel alloc] initWithFrame:CGRectMake(10, 10, 370.0/2.0, 16.0)] autorelease];
		self.label.text = @"";
